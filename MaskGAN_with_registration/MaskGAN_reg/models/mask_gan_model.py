...
class MaskGANModel(BaseModel):
    ...
    def backward_G(self):
        ...
        # 调试：检查输入是否有 NaN
        if torch.isnan(self.real_A).any() or torch.isnan(self.real_B).any():
            print(f"ERROR: Input contains NaN! real_A: {torch.isnan(self.real_A).any()}, real_B: {torch.isnan(self.real_B).any()}")
            return
        # Identity loss
        if lambda_idt > 0:
        ...
        if lambda_mask > 0:
            ...
            # 调试：检查 mask loss
            if torch.isnan(self.loss_maskA) or torch.isnan(self.loss_maskB):
                print(f"ERROR: Mask loss is NaN! maskA: {self.loss_maskA}, maskB: {self.loss_maskB}")
                print(f"  a_bg_b: min={self.a_bg_b.min():.4f}, max={self.a_bg_b.max():.4f}, has_nan={torch.isnan(self.a_bg_b).any()}")
                print(f"  mask_A: min={self.mask_A.min():.4f}, max={self.mask_A.max():.4f}, has_nan={torch.isnan(self.mask_A).any()}")
            loss_mask = lambda_mask*(self.loss_maskA + self.loss_maskB)
        else:
            self.loss_maskA = self.loss_maskB = 0
            loss_mask = 0
        
        # Shape loss（需要确保 att_rec_bg_a 存在且有效）
        if lambda_shape > 0 and hasattr(self, 'att_rec_bg_a') and self.att_rec_bg_a is not None:
            # 检查是否有 NaN
            if torch.isnan(self.att_rec_bg_a).any() or torch.isnan(self.a_bg_b).any() or torch.isnan(self.a_bg_a).any():
                print(f"WARNING: Attention maps contain NaN in shape loss calculation")
                self.loss_shapeB = torch.tensor(0.0, device=self.device)
                self.loss_shapeA = torch.tensor(0.0, device=self.device)
            else:
        ...
        # 调试：检查生成图像是否有 NaN
        if torch.isnan(self.fake_B).any() or torch.isnan(self.fake_A).any():
            print(f"ERROR: Generated images contain NaN! fake_B: {torch.isnan(self.fake_B).any()}, fake_A: {torch.isnan(self.fake_A).any()}")
            print(f"  fake_B: min={self.fake_B.min():.4f}, max={self.fake_B.max():.4f}")
            print(f"  fake_A: min={self.fake_A.min():.4f}, max={self.fake_A.max():.4f}")
        ...
        # 调试：检查基础 loss
        if torch.isnan(self.loss_G_A) or torch.isnan(self.loss_G_B):
            print(f"ERROR: GAN loss is NaN! G_A: {self.loss_G_A}, G_B: {self.loss_G_B}")
        if torch.isnan(self.loss_cycle_A) or torch.isnan(self.loss_cycle_B):
            print(f"ERROR: Cycle loss is NaN! cycle_A: {self.loss_cycle_A}, cycle_B: {self.loss_cycle_B}")
        ...
        if self.isTrain and self.lambda_reg > 0.0 and self.reg_wrapper is not None:
            ...
            # 调试：检查配准输入
            if torch.isnan(self.fake_B).any() or torch.isnan(self.real_A).any():
                print(f"ERROR: Registration input contains NaN! fake_B: {torch.isnan(self.fake_B).any()}, real_A: {torch.isnan(self.real_A).any()}")
                self.loss_reg = torch.tensor(0.0, device=self.device)
            else:
                # 清理 GPU 缓存以释放内存（注册模型需要大量内存）
                torch.cuda.empty_cache()
                
                # 如果 batch_size > 1，逐个处理样本以减少内存占用
                if self.fake_B.shape[0] > 1:
                    # 逐个处理 batch 中的样本
                    batch_size = self.fake_B.shape[0]
                    reg_losses = []
                    for i in range(batch_size):
                        fake_B_single = self.fake_B[i:i+1]  # [1, C, H, W]
                        real_A_single = self.real_A[i:i+1]   # [1, C, H, W]
                        reg_outputs = self.reg_wrapper.register_pair_with_grad(fake_B_single, real_A_single)
                        if reg_outputs is not None:
                            phi = reg_outputs.get("phi", None)
                            if phi is not None and not (torch.isnan(phi).any() or torch.isinf(phi).any()):
                                # 计算单个样本的 loss
                                if phi.dim() == 5:
                                    if len(fake_B_original_shape) == 4:
                                        target_spatial = (1, fake_B_original_shape[-2], fake_B_original_shape[-1])
                                    else:
                                        target_spatial = fake_B_original_shape[-3:]
                                    phi_resized = F.interpolate(phi, size=target_spatial, mode="trilinear", align_corners=False)
                                elif phi.dim() == 4:
                                    target_spatial = fake_B_original_shape[-2:]
                                    phi_resized = F.interpolate(phi, size=target_spatial, mode="bilinear", align_corners=False)
                                else:
                                    phi_resized = phi
                                
                                disp_sq = (phi_resized ** 2).sum(dim=1, keepdim=True)
                                ...
                                local_vals = pooled.view(1, -1)
                                k = max(1, int(local_vals.shape[1] * self.reg_topk_ratio))
                                topk_vals, _ = torch.topk(local_vals, k, dim=1)
                                local_term = topk_vals.mean()
                                
                                reg_loss = self.reg_alpha * global_term + self.reg_beta * local_term
                                reg_losses.append(reg_loss)
                        
                        # 清理中间变量
                        del fake_B_single, real_A_single
                        torch.cuda.empty_cache()
                    
                    if reg_losses:
                        # 平均所有样本的 loss
                        self.loss_reg = torch.stack(reg_losses).mean() * self.lambda_reg
                        self.loss_reg_global = torch.stack([r / self.lambda_reg for r in reg_losses]).mean().detach()
                        self.loss_reg_local = torch.stack([r / self.lambda_reg for r in reg_losses]).mean().detach()
                    else:
                        self.loss_reg = torch.tensor(0.0, device=self.device)
                        self.loss_reg_global = torch.tensor(0.0, device=self.device)
                        self.loss_reg_local = torch.tensor(0.0, device=self.device)
                    ...
                    if reg_outputs is not None:
                        ...
                        if phi is not None:
                            ...
                            if torch.isnan(phi).any() or torch.isinf(phi).any():
                                print(f"ERROR: Deformation field contains NaN/Inf! phi has_nan: {torch.isnan(phi).any()}, has_inf: {torch.isinf(phi).any()}")
                                print(f"  phi shape: {phi.shape}, min: {phi.min():.4f}, max: {phi.max():.4f}")
                                self.loss_reg = torch.tensor(0.0, device=self.device)
                            else:
                                ...
                                # 调试：检查 global_term
                                if torch.isnan(global_term) or torch.isinf(global_term):
                                    print(f"ERROR: global_term is NaN/Inf! value: {global_term}")
                                    print(f"  disp_sq: min={disp_sq.min():.4f}, max={disp_sq.max():.4f}, has_nan={torch.isnan(disp_sq).any()}")
                                    self.loss_reg = torch.tensor(0.0, device=self.device)
                                else:
        ...
        # combined loss and calculate gradients
        # 注意：这里包含了 MaskGAN 的所有 loss（mask, shape, cor）以及新增的 reg loss
        self.loss_G = self.loss_G_A + self.loss_G_B + self.loss_cycle_A + self.loss_cycle_B \
            + self.loss_idt_A + self.loss_idt_B + loss_mask + loss_shape + loss_cor + self.loss_reg
    ...
    def optimize_parameters(self):
        ...
        # 检查前向传播后的 NaN
        if torch.isnan(self.fake_B).any() or torch.isnan(self.fake_A).any():
            print("ERROR: NaN detected in fake images after forward()!")
            return
        ...
        if torch.isnan(self.loss_G) or torch.isinf(self.loss_G):
            print(f"ERROR: loss_G is NaN/Inf! loss_G={self.loss_G}")
            print(f"  loss_G_A={self.loss_G_A}, loss_G_B={self.loss_G_B}")
            print(f"  loss_cycle_A={self.loss_cycle_A}, loss_cycle_B={self.loss_cycle_B}")
            print(f"  loss_reg={self.loss_reg}")
            return
        ...
        if USE_APEX:
            ...
            torch.nn.utils.clip_grad_norm_(self.netG_B.parameters(), 1.0)
        
        # 检查梯度中的 NaN
        for name, param in self.netG_A.named_parameters():
            if param.grad is not None and torch.isnan(param.grad).any():
                print(f"ERROR: NaN gradient in G_A.{name}!")
                return
        for name, param in self.netG_B.named_parameters():
            if param.grad is not None and torch.isnan(param.grad).any():
                print(f"ERROR: NaN gradient in G_B.{name}!")
                return
        ...
        if torch.isnan(loss_D) or torch.isinf(loss_D):
            print(f"ERROR: loss_D is NaN/Inf! loss_D={loss_D}")
            print(f"  loss_D_A={self.loss_D_A}, loss_D_B={self.loss_D_B}")
            return
        
        if USE_APEX:
            with amp.scale_loss(loss_D, self.optimizer_D, loss_id=1) as scaled_loss:
                scaled_loss.backward()
            torch.nn.utils.clip_grad_norm_(amp.master_params(self.optimizer_D), 1.0)  # 修复：应该是 optimizer_D
        else:
            loss_D.backward()
            # 添加梯度裁剪以防止梯度爆炸
        ...
        for name, param in self.netD_A.named_parameters():
            if param.grad is not None and torch.isnan(param.grad).any():
                print(f"ERROR: NaN gradient in D_A.{name}!")
                return
        for name, param in self.netD_B.named_parameters():
            if param.grad is not None and torch.isnan(param.grad).any():
                print(f"ERROR: NaN gradient in D_B.{name}!")
                return
