...
class BaseModel(ABC):
    ...
    def get_current_losses(self):
        ...
        for name in self.loss_names:
            if isinstance(name, str):
                loss_val = getattr(self, 'loss_' + name)
                # 处理 tensor 和标量，确保 detach 后再转换
                if isinstance(loss_val, torch.Tensor):
                    loss_val = loss_val.detach().cpu().item() if loss_val.numel() == 1 else float(loss_val.detach().cpu().numpy())
                else:
                    loss_val = float(loss_val)
                # 检查 NaN 和 Inf
                if not (np.isfinite(loss_val) if isinstance(loss_val, (float, np.floating)) else True):
                    loss_val = float('nan')
                errors_ret[name] = loss_val
        return errors_ret
