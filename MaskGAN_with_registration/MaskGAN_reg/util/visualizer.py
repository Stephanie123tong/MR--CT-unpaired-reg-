...
def save_images(webpage, visuals, image_path, aspect_ratio=1.0, width=256):
    ...
    for label, im_data in visuals.items():
        ...
        save_path = os.path.join(image_dir, image_name)
        h, w, _ = im.shape
        # 用 PIL 而不是 OpenCV 做简单缩放，避免依赖 libGL
        if aspect_ratio != 1.0:
            if aspect_ratio > 1.0:
                new_h, new_w = h, int(w * aspect_ratio)
            else:
                new_h, new_w = int(h / aspect_ratio), w
            im_pil = Image.fromarray(im.astype(np.uint8))
            im_pil = im_pil.resize((new_w, new_h), resample=Image.BICUBIC)
            im = np.array(im_pil)
