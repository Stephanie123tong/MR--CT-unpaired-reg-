...
if __name__ == '__main__':
    ...
    for mri_path, ct_path in zip(mri_files_val,ct_files_val):
        ...
        # 对 MRI / CT 及其 mask 做一次中心裁剪，使三者形状一致，避免广播错误
        def center_crop_to(arr, target_shape):
            D, H, W = arr.shape
            d, h, w = target_shape
            sd = max((D - d) // 2, 0)
            sh = max((H - h) // 2, 0)
            sw = max((W - w) // 2, 0)
            return arr[sd:sd + d, sh:sh + h, sw:sw + w]

        min_D = min(mri.shape[0], ct.shape[0], mri_mask.shape[0], ct_mask.shape[0])
        min_H = min(mri.shape[1], ct.shape[1], mri_mask.shape[1], ct_mask.shape[1])
        min_W = min(mri.shape[2], ct.shape[2], mri_mask.shape[2], ct_mask.shape[2])
        target_shape = (min_D, min_H, min_W)

        mri      = center_crop_to(mri,      target_shape)
        mri_mask = center_crop_to(mri_mask, target_shape)
        ct       = center_crop_to(ct,       target_shape)
        ct_mask  = center_crop_to(ct_mask,  target_shape)
