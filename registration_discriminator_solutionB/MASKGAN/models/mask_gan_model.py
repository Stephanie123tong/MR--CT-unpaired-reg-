...
class MaskGANModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        if is_train:
            ...
            # Registration discriminator options (Solution B: Block-level)
            parser.add_argument('--use_registration_discriminator', action='store_true', 
                              help='enable registration discriminator')
            parser.add_argument('--lambda_reg_block', type=float, default=0.5,
                              help='weight for block-level registration discriminator loss')
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:  # define discriminators
            ...
            if hasattr(opt, 'use_registration_discriminator') and opt.use_registration_discriminator:
                ...
                # Initialize registration discriminators (Block-level)
                num_blocks = getattr(opt, 'num_blocks', 8)
                feature_dim = 4  # residual, magnitude, gradient, anomaly
                ...
                # Registration loss weights (for generator)
                self.lambda_reg_block = getattr(opt, 'lambda_reg_block', 0.5)
    ...
    def _warp_image(self, image, phi):
        """
        Warp image using deformation field phi.
        
        Args:
            image: [B, C, H, W]
            phi: [B, 2, H, W] - deformation field (x, y displacements)
        ...
        # phi[:, 0, :, :] is x-direction displacement
        # phi[:, 1, :, :] is y-direction displacement
        x_coords = x_coords + phi[:, 0, :, :]
        y_coords = y_coords + phi[:, 1, :, :]
    ...
    def backward_D_reg_A(self):
        """
        Calculate GAN loss for registration discriminator D_reg_A (Solution B).
        Selects best and worst blocks from (fake_B, real_B) registration as positive/negative samples.
        """
        if self.net_reg is None:
            self.loss_D_reg_A = torch.tensor(0.0, device=self.device)
            return
        
        # ========== Register (fake_B, real_B) ==========
        fake_B_sample = self.fake_B_pool.query(self.fake_B)
        reg_result = self.net_reg(fake_B_sample.detach(), self.real_B)
        phi = reg_result.phi_AB_vectorfield
        
        # Warp fake_B using phi to get warped image
        warped_fake_B = self._warp_image(fake_B_sample, phi)
        
        # ========== Compute block-level quality scores ==========
        block_features, block_scores = compute_block_quality_scores(
            phi,
            warped_fake_B,
            self.real_B,
        ...
        # ========== Select positive (best) and negative (worst) blocks ==========
        B = block_scores.shape[0]
        num_total_blocks = block_scores.shape[1]
        
        # Select top-k best blocks as positive samples
        # Select bottom-k worst blocks as negative samples
        k = max(1, num_total_blocks // 2)  # Use half of blocks for each
        
        # Get indices of top-k and bottom-k blocks
        _, top_indices = torch.topk(block_scores, k, dim=1)  # [B, k]
        _, bottom_indices = torch.topk(block_scores, k, dim=1, largest=False)  # [B, k]
        
        # Gather features for positive and negative samples
        # [B, k, feature_dim]
        positive_features = torch.gather(
            block_features, 
            dim=1, 
            index=top_indices.unsqueeze(-1).expand(-1, -1, block_features.shape[2])
        )
        negative_features = torch.gather(
            block_features,
            dim=1,
            index=bottom_indices.unsqueeze(-1).expand(-1, -1, block_features.shape[2])
        )
        
        # Flatten to [B*k, feature_dim] for batch processing
        positive_features = positive_features.view(B * k, -1)  # [B*k, feature_dim]
        negative_features = negative_features.view(B * k, -1)  # [B*k, feature_dim]
        
        # ========== Discriminator Forward ==========
        # Add batch dimension for discriminator: [B*k, feature_dim] -> [B*k, 1, feature_dim]
        positive_features = positive_features.unsqueeze(1)  # [B*k, 1, feature_dim]
        negative_features = negative_features.unsqueeze(1)  # [B*k, 1, feature_dim]
        
        pred_positive = self.netD_reg_A(positive_features)  # [B*k, 1, 1]
        pred_negative = self.netD_reg_A(negative_features)   # [B*k, 1, 1]
        
        # Reshape to [B*k] for loss computation
        pred_positive = pred_positive.view(B * k)  # [B*k]
        pred_negative = pred_negative.view(B * k)  # [B*k]
        
        loss_D_positive = self.criterionGAN(pred_positive, True)
        loss_D_negative = self.criterionGAN(pred_negative, False)
        
        self.loss_D_reg_A = (loss_D_positive + loss_D_negative) * 0.5
    
    def backward_D_reg_B(self):
        """
        Calculate GAN loss for registration discriminator D_reg_B (Solution B).
        Same as backward_D_reg_A but for direction B -> A.
        Selects best and worst blocks from (fake_A, real_A) registration as positive/negative samples.
        """
        if self.net_reg is None:
            self.loss_D_reg_B = torch.tensor(0.0, device=self.device)
            return
        
        # ========== Register (fake_A, real_A) ==========
        fake_A_sample = self.fake_A_pool.query(self.fake_A)
        reg_result = self.net_reg(fake_A_sample.detach(), self.real_A)
        phi = reg_result.phi_AB_vectorfield
        
        # Warp fake_A using phi to get warped image
        warped_fake_A = self._warp_image(fake_A_sample, phi)
        ...
        pred_positive = self.netD_reg_B(positive_features)  # [B*k, 1, 1]
        pred_negative = self.netD_reg_B(negative_features)   # [B*k, 1, 1]
        ...
        self.loss_D_reg_B = (loss_D_positive + loss_D_negative) * 0.5
    ...
    def backward_G(self):
        ...
        # Registration losses (Solution B: Block-level)
        if hasattr(self, 'net_reg') and self.net_reg is not None and \
           hasattr(self.opt, 'use_registration_discriminator') and self.opt.use_registration_discriminator:
            ...
            # Warp fake_B using phi_B
            warped_fake_B = self._warp_image(self.fake_B, phi_B)
            
            # Compute block-level features and scores
            block_features_B, block_scores_B = compute_block_quality_scores(
                phi_B,
                warped_fake_B,
                self.real_B,
                num_blocks=self.num_blocks,
            ...
            # Generator wants all blocks to be classified as "good"
            # Reshape block_features_B: [B, num_blocks*num_blocks, feature_dim] -> [B*num_blocks*num_blocks, 1, feature_dim]
            B = block_features_B.shape[0]
            num_total_blocks = block_features_B.shape[1]
            block_features_B_reshaped = block_features_B.view(B * num_total_blocks, -1).unsqueeze(1)  # [B*N*N, 1, feature_dim]
            
            # Discriminator prediction for all blocks
            pred_reg_B = self.netD_reg_A(block_features_B_reshaped)  # [B*N*N, 1, 1]
            pred_reg_B = pred_reg_B.view(B * num_total_blocks)  # [B*N*N]
            
            # Generator loss: all blocks should be classified as "good" (True)
            self.loss_G_reg_A = self.lambda_reg_block * self.criterionGAN(pred_reg_B, True)
            
            # For fake_A (B -> A direction)
            reg_result_A = self.net_reg(self.fake_A, self.real_A)
            phi_A = reg_result_A.phi_AB_vectorfield
            
            # Warp fake_A using phi_A
            warped_fake_A = self._warp_image(self.fake_A, phi_A)
            
            # Compute block-level features and scores
            block_features_A, block_scores_A = compute_block_quality_scores(
                phi_A,
                warped_fake_A,
                self.real_A,
                num_blocks=self.num_blocks,
            ...
            # Reshape block_features_A: [B, num_blocks*num_blocks, feature_dim] -> [B*num_blocks*num_blocks, 1, feature_dim]
            block_features_A_reshaped = block_features_A.view(B * num_total_blocks, -1).unsqueeze(1)  # [B*N*N, 1, feature_dim]
            
            # Discriminator prediction for all blocks
            pred_reg_A = self.netD_reg_B(block_features_A_reshaped)  # [B*N*N, 1, 1]
            pred_reg_A = pred_reg_A.view(B * num_total_blocks)  # [B*N*N]
            
            # Generator loss: all blocks should be classified as "good" (True)
            self.loss_G_reg_B = self.lambda_reg_block * self.criterionGAN(pred_reg_A, True)
        else:
